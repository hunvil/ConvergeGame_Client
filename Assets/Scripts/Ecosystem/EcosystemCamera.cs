...
public class EcosystemCamera : MonoBehaviour {
	...
	void Update() {
		...
		if (isZoomed) {
			...
			if (isZooming && !Mathf.Approximately(camera.fieldOfView, cameraZoomed)) {
...
//				zPos = Mathf.Clamp(zPos, cameraStartPos.z - 30, cameraStartPos.z + 30);
			...
			if (isZooming && !Mathf.Approximately(camera.fieldOfView, cameraNormal)) {
...
//			}
	...
	private bool PerformCameraEntrance() {
		...
		if (isEntering > 0 && Mathf.Abs(camera.fieldOfView - cameraNormal) > 1f) {
			camera.fieldOfView = Mathf.Lerp(camera.fieldOfView, cameraNormal, Time.deltaTime * cameraEnterSmoothing);
		} else if (isEntering < 0 && Mathf.Abs(camera.fieldOfView - cameraEnter) > 1f) {
			camera.fieldOfView = Mathf.Lerp(camera.fieldOfView, cameraEnter, Time.deltaTime * cameraEnterSmoothing);
		} else {
			isEntering = 0;
		}
	...
	public float GetEnteringProgress() {
		return Mathf.Abs(camera.fieldOfView - cameraEnter);
	}
	...
	public void TriggerEntering() {
		...
		isEntering = 1;
		camera.fieldOfView = cameraEnter;
	}
